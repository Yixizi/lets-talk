...
const NavBar = () => {
  ...
  return (
    <nav className=" flex justify-between items-center fixed z-50 px-10 gap-4 shadow-2xl w-full h-28 bg-gray-200">
      <Link
        href={"/"}
        className=" flex items-center gap-1 hover:scale-150 duration-500"
      >
        <Image src={"/assets/logo.svg"} width={60} height={60} alt="一起交流" />
      </Link>
          ...
          {navLinks.map((item) => {
            ...
            return (
              <Link
                ...
                key={item.label}
                className={cn(
                  `flex gap-4 items-center p-4 rounded-lg justify-start hover:scale-150 duration-150`,
